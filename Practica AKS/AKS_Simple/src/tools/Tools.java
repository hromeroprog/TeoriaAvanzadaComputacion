...
public class Tools {
    ...
    public static void exportCSV(String output_folder, ArrayList<BigInteger> numbers,ArrayList<BigInteger> numbers_r, ArrayList<Long> times) throws IOException {
        FileWriter writer = new FileWriter(output_folder);

        StringBuilder result = new StringBuilder("Number,NumberR,Time\n");
        for (int i = 0; i < numbers.size(); i++) {
            result.append(numbers.get(i).toString()).append(",").append(numbers_r.get(i).toString()).append(",").append(times.get(i).toString());
            if (i < numbers.size() - 1)
                result.append("\n");
        }
        writer.write(result.toString());
